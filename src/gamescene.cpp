...
void GameScene::Update(const std::vector<InputState>& inputs, uint32_t now)
{
	...
	if (_pastMapsCursor == 0)
	{
		_pastMapsCursor += _pastMaps.size();
	}
	_pastMapsCursor -= 1;
	_pastMaps[_pastMapsCursor] = (std::make_pair(now, _presentMap));
...
void GameScene::BackThroughTime()
{
	if (!_pastMaps.empty())
	{
		auto gameState = _pastMaps[_pastMapsCursor];
		_pastMapsCursor = (_pastMapsCursor + 1) % _pastMaps.size();
