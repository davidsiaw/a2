...
void game(std::shared_ptr<GameContext> context)
{
    ...
    wholeScene->AddScene(fpsScene, 100, 100);
    ...
    auto processingScene = std::make_shared<ProcessingScene>(
        [&](const InputState& inputs, Uint32 timestamp)
        {
            ...
            {
                scene->MoveCharacter(rikka, InputState::LEFT, 4, 256);
            }
			else if (inputs.GetButtonState(InputState::RIGHT))
            {
                scene->MoveCharacter(rikka, InputState::RIGHT, 4, 256);
            }

			if (inputs.GetButtonState(InputState::DOWN))
			{
				scene->MoveCharacter(rikka, InputState::DOWN, 4, 256);
			}
			else if (inputs.GetButtonState(InputState::UP))
			{
				scene->MoveCharacter(rikka, InputState::UP, 4, 256);
			}
